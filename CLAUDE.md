...
Checks & formatting:
- `nix flake check` — custom outputs (`nixosModules`, `homeManagerModules`, etc.) emit informational warnings; these are expected.
...
- `exports.nix` — imports the registries below and exposes them **both** as flake outputs (`nixosModules`, `homeManagerModules`, `overlays`) **and** as `projectOutputs` for internal use.
...
### The module → host layering

Everything composes through plain NixOS/Home-Manager modules (attrsets); there are **no custom `mkOption` definitions** — composition is done purely by importing. There is **no profile layer**: hosts import modules directly and spell out their full module list.
...
2. **Hosts** (`hosts/<name>/config.nix`, `home.nix`): a flat `imports` list naming every module the host wants (`outputs.nixosModules.<domain>.<name>`, `outputs.homeManagerModules.<domain>.<name>`) plus host-specific hardware/overrides. Each host is explicit — no bundles, no `builtins.attrValues`, so what a host gets is readable from its own file. `hosts/hosts.nix` is the single source of truth mapping host name → `{ system, nixosModules, homeModule, enableHome, ... }`.

**To add a new module:** create the `.nix` file under the right domain directory — it is auto-discovered by `flake/lib/registry.nix` (attr name = filename minus `.nix`; a subdirectory with a `default.nix` is imported verbatim as a single module; one without recurses into a nested registry). Then add `outputs.nixosModules.<domain>.<name>` to the `imports` of **each host** that should get it — nothing picks modules up automatically. It goes live on the next rebuild once it is `git add`ed (untracked files are invisible to flake evaluation) — keep work-in-progress modules outside `modules/`.
